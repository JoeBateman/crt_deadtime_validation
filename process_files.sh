...
if [ "$START_STAGE" == "decode" ] || [ -z "$START_STAGE" ]; then
    ...
    . ./tools/run_fcl_batch.sh "decode" $SOURCE_PATH $TEMP_PATH $FILE_LIST_PATH $N_files
fi

if [ "$START_STAGE" == "reco" ] || [ "$START_STAGE" == "decode" ] || [ -z "$START_STAGE" ]; then
    # Use the reco stage for run_fcl_batch.sh
    echo "Running reco stage"
    . ./tools/run_fcl_batch.sh "reco" $TEMP_PATH"/decode" $TEMP_PATH $FILE_LIST_PATH $N_files
    . ./tools/split_reco_file_lists.sh $FILE_LIST_PATH"/files_reco.list"
